...
pub fn game(mut runtime_blob: RuntimeFunctionBlob) -> RuntimeFunctionBlob {
    ...
    for tries in runtime_blob.settings.min_range - 1..runtime_blob.settings.max_tries {
        ...
        match numeric_input(&runtime_blob.comunication.msg).cmp(&secret_number) {
    ...
    cls_title();
    runtime_blob.comunication.msg = format!("You loose! The secret number was {}", secret_number); //Indique au joueur qu'il a perdu et quel était le numéro cherché.
