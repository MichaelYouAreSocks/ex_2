...
pub fn score_game_layout(high_scores: &[String]) -> Result<String, ErrFormat> {
    ...
    while score_rank < high_scores.len() / 3 {
...
pub fn score_file_layout(high_scores: &[String]) -> Result<String, ErrFormat> {
    ...
    let amount_scores = high_scores.len() / 3;
    let mut layout: String = format!(
        "{}\n{}\n{}\n{}\n{}",
    ...
    while score_rank < amount_scores + 1 {
        ...
        for column_num in (score_rank * 3 - 3)..=(score_rank * 3 - 1) {
            layout += format!("\t{}", high_scores[column_num].as_str()).as_str();
            layout += match high_scores[column_num].chars().count() {
                0..=3 => "\t\t|",
                4..=7 => "\t|",
                _ => "|",
            }
